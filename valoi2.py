...
if 'target_value' not in st.session_state:
    st.session_state.update({
        ...
        'check_status': 'idle', 'play_sound': None
    })
...
with col_main:
    st.title("🎧 Luyện Nghe Nhật - Trung")
    
    lang = st.selectbox("🌐 Ngôn ngữ", ["Tiếng Nhật (ja)", "Tiếng Trung (zh-CN)"])
    mode = st.selectbox("📊 Loại hình", ["Số đếm", "Ngày tháng", "Phần trăm (%)", "Khoảng thời gian & Thứ", "Thời gian tổng hợp"])
    ...
    if mode in ["Số đếm", "Phần trăm (%)"]:
        ...
        with c4: limit_option = st.select_slider("🚀 Giới hạn", options=[100, 1000, 10000, 100000, 1000000, 100000000, 1000000000, 9999999999999], value=100000)

    if st.button("🔔 PHÁT NỘI DUNG MỚI", use_container_width=True):
        lang_key = "ja" if "Nhật" in lang else "zh-CN"
        
        # Logic tạo nội dung (giữ nguyên các bản trước)
        ...
        st.session_state.update({'display_answer': False, 'check_status': 'idle', 'input_key': st.session_state.input_key + 1})
        
        # --- FIX CHO IPHONE: TẠO FILE THẬT ---
        ...
        tts.save("temp_audio.mp3") # Lưu thành file vật lý để iPhone dễ đọc hơn

    # --- HIỂN THỊ THANH AUDIO ---
